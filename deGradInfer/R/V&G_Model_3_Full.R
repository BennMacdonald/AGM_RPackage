### This script creates the Vyshemirsky And Girolami model 3. It sets up the 
...
### Species: There are 3 species and 4 species from models 1 and 4
...
### Parameters: There are 4 kinetic parameters
...
gradientsVGModel3Full <- function(params,X,timepoints=NULL,auxVars,species
					  )
{

	rat1 <- (params[3]*X[,3]*X[,1])/(params[1]+X[,3])

	rat2 <- (params[4]*X[,5])/(params[2]+X[,5])
...
odeVGModel3Full <- function(t,X,params)
{
...
### Creation of simulation VG model 3. It uses the function "ode" in the cran
	### package "deSolve" to solve the odeV&G model function above. xInit
...
simulateVGModel3Full <- function(xInit,t,params,...)
{
	solution <- ode(xInit,t,odeVGModel3Full,params,...)
